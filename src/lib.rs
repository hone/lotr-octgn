...
#[cfg(not(test))]
const OCTGN_GIT_URL: &str = "https://github.com/GeckoTH/Lord-of-the-Rings.git";
#[cfg(test)]
const OCTGN_GIT_URL: &str = "fixtures/octgn";
...
fn update_octgn_git_dir(dir: &Path) -> Result<(), git2::Error> {
...
mod tests {
    ...
    fn test_fetch_or_update_octgn_git_dir_new() {
        ...
        let result = fetch_or_update_octgn_git_dir(tmp_dir.path());
        assert!(result.is_ok());
    ...
    fn test_fetch_or_update_octgn_git_dir_existing() {
        ...
        dir::copy(OCTGN_GIT_URL, &tmp_dir, &copy_options).unwrap();
        dir::move_dir(
        ...
        let result = fetch_or_update_octgn_git_dir(&cache_git_dir);
        assert!(result.is_ok());
    ...
    fn test_fetch_or_update_octgn_git_dir_bad_dir() {
        let tmp_dir = TempDir::new("octgn").unwrap();
        let repo = Repository::clone(OCTGN_GIT_URL, &tmp_dir.path()).unwrap();
